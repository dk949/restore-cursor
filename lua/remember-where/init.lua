...
---@param opts Options
---@return Options
local function fillDefaultOpts(opts)
    if not opts then return default_options end
    opts.pat = vim.tbl_deep_extend("keep", typeCheckPattern(opts.pat), default_patterns)
    if opts.event == nil then opts.event = default_options.event end
    return opts
end
...
return function(opts)
    ...
    if opts.pat.only then opts.pat.override_ignore = nil end

    local M = {}

    local function remember(ev)
        if not shouldRun() then return end
        if not bufMatches(ev.file, vim.bo.filetype, opts.pat) then return end
        local line = vim.fn.line("'\"")
        if line < 1 or line > vim.fn.line("$") then return end
        if opts.cb_pre then opts.cb_pre() end
        vim.cmd[[keepjumps norm g`"]]
        if opts.cb_post then opts.cb_post() end
    end

    function M.installHandler()
        M.group = vim.api.nvim_create_augroup("remember_where_group", { clear = true })
        M.autocmd = vim.api.nvim_create_autocmd(opts.event, {
            group = M.group,
            callback = remember,
